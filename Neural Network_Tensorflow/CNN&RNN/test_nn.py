...
def test_toy_rnn(capsys):
    ...
    assert output_activation(model) == keras.activations.linear
...
def test_image_cnn(capsys):
    ...
    assert output_activation(model) == keras.activations.softmax
...
def test_text_rnn(capsys):
    ...
    assert output_activation(model) == keras.activations.sigmoid
...
def layers(model: keras.models.Model):
    return [x.layer if isinstance(x, keras.layers.Wrapper) else x
            for x in model.layers]


def is_convolution(layer: keras.layers.Layer):
    return layer.__class__.__name__.startswith('Conv')


def is_recurrent(layer: keras.layers.Layer):
    # return isinstance(layer, keras.layers.RNN)  # no longer sufficient in TensorFlow
    return (isinstance(layer, keras.layers.RNN) or
            isinstance(layer, keras.layers.Bidirectional) or
            isinstance(layer, keras.layers.LSTM) or
            isinstance(layer, keras.layers.GRU))
...
def output_activation(model: keras.models.Model):
